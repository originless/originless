...
describe('getConfiguration', () => {
  ...
  it('uses the overrides', async () => {
    ...
    assert.deepStrictEqual(await getConfiguration({ plugins: ['@originless/plugin-example-1'] }), {
      include: ['**/*.api.ts'],
      exclude: ['node_modules/**'],
      plugins: ['@originless/plugin-example-1'],
    })
  })
  ...
  it('deep merges with uniqueness', async () => {
    ...
    assert.deepStrictEqual(await getConfiguration({ plugins: ['@originless/plugin-example-2'] }), {
      include: ['**/*.api.ts'],
      exclude: ['node_modules/**'],
      plugins: ['@originless/plugin-example-1', '@originless/plugin-example-2'],
    })
